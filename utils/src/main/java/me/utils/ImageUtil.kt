...
object ImageUtil {
    ...
    fun bytes2Bitmap(bytes: ByteArray?): Bitmap? =
            if (bytes == null || bytes.isEmpty()) null else BitmapFactory.decodeByteArray(bytes, 0, bytes.size)
    ...
    fun bitmap2Drawable(bitmap: Bitmap?): Drawable? =
            if (bitmap == null) null else BitmapDrawable(Utils.context.resources, bitmap)
    ...
    fun drawable2Bytes(drawable: Drawable?, format: CompressFormat): ByteArray? =
            if (drawable == null) null else bitmap2Bytes(drawable2Bitmap(drawable), format)
    ...
    fun getBitmap(file: File?): Bitmap? =
            if (file == null) null else BitmapFactory.decodeFile(file.absolutePath)
    ...
    fun getBitmap(filePath: String): Bitmap? =
            if (isSpace(filePath)) null else BitmapFactory.decodeFile(filePath)
    ...
    fun getBitmap(`is`: InputStream?): Bitmap? =
            if (`is` == null) null else BitmapFactory.decodeStream(`is`)
    ...
    fun getBitmap(data: ByteArray, offset: Int): Bitmap? =
            if (data.isEmpty()) null else BitmapFactory.decodeByteArray(data, offset, data.size)
    ...
    fun getBitmap(@DrawableRes resId: Int): Bitmap {
        val drawable = ContextCompat.getDrawable(Utils.context, resId)
        val canvas = Canvas()
    ...
    fun getBitmap(fd: FileDescriptor?): Bitmap? =
            if (fd == null) null else BitmapFactory.decodeFileDescriptor(fd)
     ...
     * @return 圆形图片
     */
    fun toRound(src: Bitmap, recycle: Boolean): Bitmap? = toRound(src, 0, 0, recycle)
    ...
    fun renderScriptBlur(src: Bitmap?,
        ...
        val ret = if (recycle) src else src!!.copy(src.config, true)
        try {
            rs = RenderScript.create(Utils.context)
            rs!!.messageHandler = RenderScript.RSMessageHandler()
            val input = Allocation.createFromBitmap(rs, ret, Allocation.MipmapControl.MIPMAP_NONE, Allocation
                    .USAGE_SCRIPT)
    ...
    private fun isEmptyBitmap(src: Bitmap?): Boolean =
            src == null || src.width == 0 || src.height == 0
    ...
    fun compressByScale(src: Bitmap, newWidth: Int, newHeight: Int): Bitmap? =
            scale(src, newWidth, newHeight, false)
